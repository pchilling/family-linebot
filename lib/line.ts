  ...
  switch (action) {
    ...
    case 'faq_return':
      return [
        '📦 退換貨政策',
        '',
        '親愛的顧客,感謝您選擇我們的商品!為提供更好的購物體驗,請在下單前詳細閱讀以下退換貨政策。',
        '',
        '【一、七天鑑賞期】',
        '依台灣《消費者保護法》,網路購買商品享有七天鑑賞期,鑑賞期並非試用期。',
        '商品須保持全新及包裝完整(含本體、配件、贈品、保固書、原廠包裝盒與發票等),否則可能影響退貨權益。',
        '',
        '【二、對商品有疑問】',
        '下單前請仔細確認商品資訊,如有任何疑問歡迎聯繫客服,我們將竭誠為您解答。',
        '',
        '【三、不適用退換貨的商品】',
        '• 書籍與雜誌:因涉及智慧財產權,售出後恕不接受退換貨',
        '• 客製化商品:所有經客製化設計的商品,均不接受退換貨',
        '• 拆封後無法恢復原狀的商品:例如個人衛生用品、易變質食品等',
        '',
        '【四、退貨須知】',
        '若商品符合退貨條件,請於收到商品後七日內聯繫客服,經審核確認後,我們將提供退貨指引。',
        '退回商品須保持完整包裝;若因人為因素造成商品受損,將無法接受退貨申請。',
      ].join('\n');
    ...
    case 'faq_warranty':
      return [
        '🛡 保固政策',
        '',
        '本產品自訂購日起享有三個月保固服務,逾期恕不提供保固。',
        '保固期間內,若因非人為因素造成產品故障或品質瑕疵,我們將提供免費維修或更換服務。',
        '',
        '【以下情況不適用保固】',
        '• 人為損壞或不當使用',
        '• 外力撞擊或進水',
...
function buildNewsFooter(links: { label: string; url: string }[]): messagingApi.FlexBox {
  return {
    ...
    layout: 'vertical' as const,
    spacing: 'sm' as const,
    contents: links.slice(0, 3).map((l) => ({
      type: 'button' as const,
      style: 'primary' as const,
      color: '#18181b',
      height: 'sm' as const,
      action: { type: 'uri' as const, label: l.label.slice(0, 20), uri: l.url.trim() },
    })),
  };
}
