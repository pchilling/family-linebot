...
// timestamptz → 'YYYY-MM-DDTHH:MM'(Asia/Taipei,給 datetime-local 用)
...
export default async function ClassesPage() {
  const [regions, classes] = await Promise.all([getRegions(), getClasses()]);

  return (
    <main style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 22, marginBottom: 20 }}>本月課程管理</h1>
      ...
      <section style={section}>
        ...
        <form action={createClass} style={formGrid}>
          ...
          <label style={label}>
            <span style={labelText}>課程名稱</span>
            <input name="name" required style={input} />
          </label>
          <label style={label}>
            <span style={labelText}>時間</span>
            <input
              name="scheduled_at"
              type="datetime-local"
              required
          ...
          <label style={label}>
            <span style={labelText}>講師</span>
            <input name="instructor" style={input} />
          </label>
          <label style={label}>
            <span style={labelText}>時長(分)</span>
            <input
              name="duration_min"
              type="number"
              defaultValue={90}
          ...
          <label style={label}>
            <span style={labelText}>價格 TWD(收費才填)</span>
            <input name="price_twd" type="number" style={input} />
          </label>
          <label style={{ ...label, gridColumn: '1 / -1' }}>
            <input name="is_paid" type="checkbox" /> 收費課
          </label>
            ...
            <li key={c.id} style={listItem}>
              <form action={updateClass} style={formGrid}>
                ...
                <label style={label}>
                  ...
                  <input name="name" defaultValue={c.name} style={input} />
                </label>
                <label style={label}>
                  <span style={labelText}>時間</span>
                  <input
                    name="scheduled_at"
                    type="datetime-local"
                    defaultValue={toLocalInput(c.scheduled_at)}
                    style={input}
                  />
                </label>
                <label style={label}>
                ...
                <label style={label}>
                  <span style={labelText}>價格</span>
                  <input
                    name="price_twd"
                    type="number"
                    defaultValue={c.price_twd ?? ''}
                    style={input}
                  />
                </label>
